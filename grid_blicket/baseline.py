...
def main():
    ...
    wandb.init(project="grid_blicket_env", 
            entity="dongyanl1n", 
            name=f"{args.env_name}-ppo-{datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}",
    ...
    rollouts.to(device)
    obs = envs.reset()  # torch.Size([num_processes, 3, 56, 56])
    ...
    for j in range(10000):  # one rollout/episode per update
        ...
        # shapes:
        # obstotal: torch.Size([ep_length+1, 16, 3, 56, 56])
        # rewardtotal: torch.Size([ep_length, 16, 1])
        # recurrent_hidden_statestotal: torch.Size([ep_length+1, 16, 1])
        # actiontotal: torch.Size([ep_length, 16, 1])
        # maskstotal: torch.Size([ep_length+1, 16, 1])
        ...
        print(f'update {j} | extrinsic reward: {rewardstotal.sum(0).mean().cpu().detach().numpy()}')
