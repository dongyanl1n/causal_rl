...
conspec_prototypes = prototypes(input_size=conspec_encoder.recurrent_hidden_state_size, hidden_size=100, 
                            num_prototypes=num_prototypes, device=device)
conspec_prototypes.to(device)

# load weights
base_path = '/network/scratch/l/lindongy/blicket_objects_env/policy/conspec_ckpt/MiniWorld-PutNext-v0-conspec-lr0.0007-intrinsR0.5-lrConSpec0.002-entropy0.02-seed2'
ckpt_epi = 9999
...
frozen_sfbuffer_path = f'conspec_rollouts_frozen_epoch_{ckpt_epi}.pth'
buffer_path = f'buffer_epoch_{ckpt_epi}.pth'
data_path = f'conspec_rollouts_epoch_{ckpt_epi}.pth'

# cos_full_path = os.path.join(base_path, 'data', cos_sim_path)
frozen_sfbuffer_full_path = os.path.join(base_path, frozen_sfbuffer_path)
buffer_full_path = os.path.join(base_path, buffer_path)
data_full_path = os.path.join(base_path, data_path)
...
obs_batch = rollouts['obs'][:-1].reshape(250*4, 3, 60, 80)
recurrent_hidden_states_batch = rollouts['hidden_states'][:-1].reshape(250*4, -1)
masks_batch = rollouts['masks'][:-1].reshape(250*4, 1)
actions_batch = rollouts['actions'].reshape(250*4, 1)

# Equivalent to self.encoder.retrieve_hiddens
hidden = conspec_encoder.retrieve_hiddens(obs_batch, recurrent_hidden_states_batch, masks_batch)
hidden = hidden.reshape(250, 4, -1)
cos_max_score, max_inds, _, cos_scores, _ = conspec_prototypes(
    hidden, -1, loss_ortho_scale=0.1
...
obs_batch = obs_batch.reshape(250, 4, 3, 60, 80)
...
plot_high_cos_obs(cos_scores, obs_batch, threshold=threshold, save_path=os.path.join(base_path,'prototype_obs'))
